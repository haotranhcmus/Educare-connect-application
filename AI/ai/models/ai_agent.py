...
class AIAgent(models.Model):
    ...
    def _post_ai_response(self, channel, message):
        ...
        channel.sudo().with_context(ai_bot_post=True).message_post(
    ...
    def _build_system_context(self, extra_system_context: str = ""):
        ...
        # V17: Use self.env.user.tz instead of self.env.tz
        user_tz = self.env.user.tz or self.env.context.get('tz') or 'UTC'
        system_content += f"\nAll record data timestamps are in UTC. In responses, convert them to {user_tz}"
    ...
    def _parse_domain(self, model_name, domain_json_str: str | None):
        ...
        try:
            ...
            expression.normalize_domain(domain_array)
            return domain_array
    ...
    def _get_available_models(self) -> str:
        ...
        # Get ir.model records for allowed models, excluding transient
        # V17: Use standard list domain, removed 'abstract' field (not in v17 ir.model)
        search_domain = [
        ...
        # V17: Use load_menus() instead of load_web_menus()
        all_menus = self.env["ir.ui.menu"].load_menus(False)
        root_menu_ids = all_menus.get("root", {}).get("children", [])  # This is ordered by sequence

        # Create app name to sequence mapping
        app_sequence = {}
        for idx, menu_id in enumerate(root_menu_ids):
            if menu_id in all_menus:
                app_menu = self.env["ir.ui.menu"].browse(menu_id)
                if app_menu.exists():
                    # Get the technical name from xmlid
                    xmlid = app_menu.get_external_id().get(app_menu.id, "")
                    app_name = xmlid.split(".")[0] if xmlid else ""
                    if app_name:
    ...
    def _ai_tool_open_menu_list(self, menu_id, model_name, selected_filters, selected_groupbys, search, custom_domain=None):
        ...
        # V17: Parse action string "ir.actions.act_window,ID"
        action_id = self._get_action_id(menu_id)
        ...
        # V17: 'list' view is called 'tree' in action.views
        if "list" not in available_views and "tree" not in available_views:
        ...
        # V17: Use bus.bus._sendone instead of user._bus_send
        self.env['bus.bus']._sendone(self.env.user.partner_id, "AI_OPEN_MENU_LIST", bus_data)
    ...
    def _ai_tool_open_menu_kanban(self, menu_id, model_name, selected_filters, selected_groupbys, search, custom_domain=None):
        ...
        # V17: Use bus.bus._sendone instead of user._bus_send
        self.env['bus.bus']._sendone(self.env.user.partner_id, "AI_OPEN_MENU_KANBAN", bus_data)
    ...
    def _ai_tool_open_menu_pivot(self, menu_id, model_name, selected_filters, row_groupbys, col_groupbys, measures, search, custom_domain=None):
        ...
        # V17: Use bus.bus._sendone instead of user._bus_send
        self.env['bus.bus']._sendone(self.env.user.partner_id, "AI_OPEN_MENU_PIVOT", bus_data)
    ...
    def _ai_tool_open_menu_graph(
        ...
        # V17: Use bus.bus._sendone instead of user._bus_send
        self.env['bus.bus']._sendone(self.env.user.partner_id, "AI_OPEN_MENU_GRAPH", bus_data)

    def _ai_tool_compute_report_measures(self, action_id, model_name=None):
        """Compute available measures for a given action.
        
        Args:
            action_id: The ID of the action
            model_name: Optional model name for validation. If not provided or doesn't match,
                       the action's model will be used automatically.
        
        Returns:
            CSV string with available measures
        """
        action = self.env["ir.actions.act_window"].browse(action_id)
        if not action.exists():
            raise ValueError(f"The action with ID {action_id} does not exist.")

        action_dict = action._get_action_dict()
        actual_model = action_dict.get("res_model")
        
        if not actual_model:
            raise ValueError(f"The action with ID {action_id} has no model associated.")
        
        if actual_model not in self.env:
            raise ValueError(f"Model '{actual_model}' not found in the system.")
        
        # If model_name provided but doesn't match, log a warning and use action's model
        if model_name and model_name != actual_model:
            _logger.warning(
                "AI tool compute_report_measures: provided model_name '%s' doesn't match "
                "action's model '%s'. Using action's model instead.",
                model_name, actual_model
            )
        
        # Always use the action's actual model
        model = actual_model
    ...
    def _get_action_id(self, menu_id):
        ...
        if not menu.exists():
            raise ValueError(f"Menu with ID {menu_id} does not exist.")
        action_str = menu.get("action")
        if not action_str:
            raise ValueError(f"Menu with ID {menu_id} has no action.")
        try:
            action_model, action_id_str = action_str.split(',')
            return int(action_id_str)
        except (ValueError, AttributeError):
            raise ValueError(f"Menu with ID {menu_id} has invalid action format.")