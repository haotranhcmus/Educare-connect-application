...
from odoo.addons.ai.utils.mail.discuss import Store
...
class DiscussChannel(models.Model):
    ...
    def create_ai_draft_channel(
        ...
        # Build Store data for frontend - use _channel_info() to get full data with members
    ...
    # V17: Override _channel_basic_info to add ai_agent_id for AI chat channels
    def _channel_basic_info(self):
        info = super()._channel_basic_info()

        # Add ai_agent_id for AI chat channels
        if self.channel_type == 'ai_chat' and self.sudo().ai_agent_id:
            info['ai_agent_id'] = self.sudo().ai_agent_id.id

        return info

    def _notify_get_recipients(self, message, msg_vals, **kwargs):
        """Override to skip mail notifications for AI bot posts."""
        # Skip creating mail.notification records for AI bot posts
        # This improves performance by not creating unnecessary notification records in DB
        if self.env.context.get('ai_bot_post'):
            return []
