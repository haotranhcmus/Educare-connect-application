...
class MailRenderMixinAI(models.AbstractModel):
    ...
    def _render_template(self, template_src, model, res_ids, engine='inline_template', add_context=None, options=None):
        # Strip eval_ai_prompts before calling super — base validates against a hardcoded set
        eval_ai = (options or {}).get("eval_ai_prompts")
        super_options = {k: v for k, v in (options or {}).items() if k != 'eval_ai_prompts'} or None
    ...
    def _render_field(self, field, res_ids, engine='inline_template',
                      ...
                      add_context=None, options=None, **kwargs):
        ...
        return super()._render_field(
            ...
            compute_lang=compute_lang, set_lang=set_lang,
            add_context=add_context, options=options, **kwargs,
        )
