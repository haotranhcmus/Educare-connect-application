...
class LLMApiService:
    ...
    def _request_llm_silent(
        ...
        if tools:
            # Copy only the schema part, not the entire tools dict with partial functions and Odoo objects
            # Tools structure: {tool_name: (description, allow_end_message, callable, schema)}
            tools_copy = {}
            for tool_name, (description, allow_end_message, callable_func, tool_parameter_schema) in tools.items():
                # Deep copy only the schema to avoid modifying the original
                schema_copy = copy.deepcopy(tool_parameter_schema)
                if allow_end_message and "__end_message" not in schema_copy["properties"]:
                    schema_copy["properties"]["__end_message"] = {
                        "type": "string",
                ...
                if "__end_message" in schema_copy["properties"] and "__end_message" not in schema_copy["required"]:
                    schema_copy["required"].append("__end_message")
                # Keep the original callable and other values, only replace schema with the modified copy
                tools_copy[tool_name] = (description, allow_end_message, callable_func, schema_copy)
            tools = tools_copy
