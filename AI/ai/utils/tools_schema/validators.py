...
def validate_params_llm_values_with_schema(instance, schema, required_parameters, env):
    ...
    for required in required_parameters:
        if required not in instance:
            raise ValidationError(_("Could you please provide info about '%s' as it is required to process your request", required))

    instance.update({param: None for param in schema if param not in instance})

    for name, value in instance.items():
        if name not in schema:
            raise ValidationError(_("Missing definition for %(name)s", name=name))
        ...
        if (enum := schema[name].get('enum')) and value not in enum:
            raise ValidationError(_("Wrong value %(value)s, should be in: %(enum)s", value=value, enum=", ".join(map(str, enum))))
