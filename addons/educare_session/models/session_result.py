...
class EducareSessionResult(models.Model):
            ...
            if r.result_type in ("partial_interval", "momentary_time_sample"):
                if (r.total_trials or 0) < 5:
                    raise ValidationError(
                        _(
                            "Method %s usually requires at least 5 intervals/checks. "
                            "Please verify total_trials."
                        )
                        % r.result_type
                    )
    ...
    def _compute_trial_labels(self):
        ...
        for r in self:
            c_label, t_label = labels.get(
                r.result_type, ("Correct Trials", "Total Trials")
            )
