...
class EducareDailyReport(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if not vals.get('name'):
                ...
                date_str = str(
                    vals.get('report_date') or fields.Date.today()
                ).replace('-', '')
    ...
    def _compute_session_summary(self):
        for report in self:
            ...
            for r in results:
                if r.total_trials > 0:
                    acc_lines.append(
                        f'- {r.objective_id.name}: {r.accuracy_pct:.0f}%'
                    )
    ...
    def _format_parent_email_body(self):
        ...
        if self.achievements:
            sections.append(
                f"<h3>Achievements</h3><p>{self.achievements}</p>"
            )
        ...
        if self.highlight_moment:
            sections.append(
                f"<h3>Special moment</h3><p>{self.highlight_moment}</p>"
            )
