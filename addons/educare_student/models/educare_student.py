...
class EducareStudent(models.Model):
    ...
    secondary_parent_name = fields.Char(string='Secondary Parent/Guardian Name', size=128)
    secondary_parent_relation = fields.Selection([('father', 'Father'), ('mother', 'Mother'), ('guardian', 'Guardian')], string='Secondary Relation')
    secondary_parent_phone = fields.Char(string='Secondary Parent/Guardian Phone', size=20)
    emergency_contact_name = fields.Char(string='Emergency Contact Name', size=128)
    emergency_contact_phone = fields.Char(string='Emergency Contact Phone', size=20, required=True)
    emergency_contact_relation = fields.Char(string='Emergency Contact Relation', size=64)
    home_address = fields.Text(string='Home Address')
    preferred_contact_method = fields.Selection([('phone', 'Phone'), ('email', 'Email'), ('app', 'App'), ('zalo', 'Zalo')], string='Preferred Contact Method')
    receive_daily_report = fields.Boolean(string='Receive Daily Report', default=True)

    # ──  Tab 3: Diagnosis & Medical Fields ───────────────────────────────────────────────────────
    primary_diagnosis = fields.Selection([('autism', 'Autism Spectrum Disorder (ASD)'), ('adhd', 'ADHD'), ('developmental_delay', 'Developmental Delay'), ('speech_delay', 'Speech Delay'), ('intellectual_disability', 'Intellectual Disability'), ('cerebral_palsy', 'Cerebral Palsy'), ('down_syndrome', 'Down Syndrome'), ('other', 'Other')], required=True, tracking=True, index=True, string="Primary Diagnosis")
    diagnosis_detail = fields.Text(string='Diagnosis Details')
    diagnosis_date = fields.Date(string='Diagnosis Date')
    diagnosed_by = fields.Char(string='Diagnosed By', size=128, help="Name of the diagnosing specialist or organization.")
    secondary_diagnosis_ids = fields.Many2many('educare.diagnosis', string='Secondary Diagnoses', relation='student_diagnosis_rel')
    ...
    medical_alert_detail = fields.Text(string='Medical Alert Details', help='If Medical Alert is checked, provide details here.', tracking=True)
        ...
        for student in self:
            ...
            if student.enrollment_date and student.date_of_birth:
                if student.enrollment_date < student.date_of_birth:
                    raise ValidationError('Enrollment date cannot be before birth date!')
            if student.graduation_date and student.enrollment_date:
                if student.graduation_date <= student.enrollment_date:
                    raise ValidationError('Graduation date must be after enrollment date!')
    ...
    def _onchange_center_id(self):
        ...
        if self.center_id:
            ...
            if self.assigned_teacher_id:
                ...
                if profile and profile.center_id and profile.center_id != self.center_id:
